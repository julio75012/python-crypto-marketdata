...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["E", "T"],
        default="E",
        help="Use event time (E) or transaction time (T) for timestamps",
    )
    ...
    def get_timestamp(data, time_field):
        if time_field == "E":
            return int(data["E"]) * 1000
        else:
            return int(data["T"]) * 1000
        ...
        while True:
            ...
            if data is not None:
                ...
                if evt == "trade":
                    ...
                    exch_timestamp = get_timestamp(data, args.time)
                    if correct_exch_timestamp and exch_timestamp < prev_exch_timestamp:
                        print(
                            f"Correcting timestamp from {exch_timestamp} to {prev_exch_timestamp}: {line}"
                        )
                    ...
                    funding_rate = data["r"]
                    rows.append(
                        [100, prev_exch_timestamp, local_timestamp, 0, float(index), 0]
                    )
                    rows.append(
                        [
                            101,
    ...
    snapshot = []
    snapshot += [
        [4, exch_timestamp, local_timestamp, 1, float(bid), float(qty)]
        for bid, qty in sorted(bid_depth.items(), key=lambda v: -float(v[0]))
    ]
    snapshot += [
        [4, exch_timestamp, local_timestamp, -1, float(ask), float(qty)]
